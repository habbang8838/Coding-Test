...
export default function RoomClient({
  ...
  useEffect(() => {
    const timer = setInterval(() => setNow(dayjs().tz("Asia/Seoul")), 1000);
    return () => clearInterval(timer);
  }, []);

  // ✅ 클라이언트 입장 시점이 곧 startAt
  const startAt = useRef(dayjs().tz("Asia/Seoul")).current;
  const endAt = startAt.add(30, "minute");   // 항상 30분으로 고정
